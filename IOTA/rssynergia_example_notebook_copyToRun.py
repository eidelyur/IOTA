...
# Beam is matched to a generalized KV distribution with normalized emittance ????,??=0.3 mm-mrad
...
# and a fixed "tune" advance of ????=????=0.3 mm-mrad.
...
# the 1st invariant, ??0. To produce the same normalized emittance of ????,??=0.3 mm-mrad for the
# injected proton beam, we construct a bunch with fixed ??0=9.74 mm-mrad.
...
for index in range(len(opts.emits)):
    ...
    print "Constructed bunch with {} macroparticles, having mean H: {} and std:{}%" \
          .format(opts.macro_particles, bunch_mean,bunch_var)
